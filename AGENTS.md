...
the fixture's own six-decimal rounding and not a step more, because both sides
run the same ONNX Runtime version against the same weights; the observed worst
...
full-bandwidth microphone, are settled by `:app`'s `AudioCheckActivity` plus
`android/tools/check_capture.py` — the latter because a band-limited capture
*sounds fine* and it takes a spectrum to see the cliff.
...
- **Playback must be written in small slices or barge-in is a lie.**
  ...
  being written.
- **The Android side keeps the slicing, for a different reason, and its
  `flush()` needs a sandwich.** `AudioTrack.flush()` genuinely drops the
  device-side buffer, which is the part PortAudio cannot do — but it is a no-op
  on a track that is playing, so it has to be `pause()` → `flush()` → `play()`.
  And a `WRITE_BLOCKING` write cannot be abandoned, so handing over a whole
